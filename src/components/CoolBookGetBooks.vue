...
<div class="container">
  ...
  <div class="row">
    <div class="col-3" style="background-color:#C7C1C0"><h3 class="title-5">Mes types</h3>
    <li style="color:black" v-for="genre in allGenres"  :key="genre.id" >
      <ul style="background-color:#959393" @click="getSortedByGenre(genre.id)" >{{genre.label}}</ul>
    </li>
    </div>

    <div class="col-1"></div>
    <div class="col-8"> <h3 class="title-5">Mes livres</h3>
       <div v-for="book in allBooks" :key="book.id" class="card mb-3" style="max-width: 640px;">
        <div class="row no-gutters">
          ...
          <div class="col-md-8">
            <div class="card-body">
              ...
              <p class="card-button"><button type="button" class="btn btn-info" @click="goToOneBook(book.id)">Consulter</button></p>
            </div>
          </div>
        </div>
      </div>
...
name: 'CoolBookGetBooks',
  ...
  methods:{
    ...
    getSortedByGenre(idGenre){
      console.log('oui')
      this.allBooks = this.allBooks.filter((book)=>{
                  return    book.genre.id = idGenre
              })
    }
  },
    async created(){
